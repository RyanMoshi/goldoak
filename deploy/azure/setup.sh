#!/usr/bin/env bash
#
# One-shot setup for the Super Agent WhatsApp gateway on an Azure virtual
# machine (Ubuntu 22.04 or 24.04, x86 or Arm).
#
#   curl -fsSL https://raw.githubusercontent.com/RyanMoshi/goldoak/main/deploy/azure/setup.sh -o setup.sh
...
# browser on a small VM, writes the compose stack, and starts it behind HTTPS.
...
# Azure's instance metadata service, which answers only from inside the VM.
imds() { curl -fsS -H 'Metadata: true' --max-time 3 "$IMDS?api-version=2021-02-01" 2>/dev/null || true; }
...
if [ -n "$META" ] && command -v jq >/dev/null 2>&1; then
  VMSIZE=$(printf '%s' "$META" | jq -r '.compute.vmSize // empty')
  VMLOC=$(printf '%s' "$META" | jq -r '.compute.location // empty')
  [ -n "$VMSIZE" ] && log "VM size: $VMSIZE in $VMLOC"
fi

MEM_MB=$(free -m | awk '/^Mem:/{print $2}')
log "Memory: ${MEM_MB} MB"

# A B1s has 1 GB. Chromium wants more, so the swap below is not optional there
# - it is the difference between a working gateway and one the kernel kills
# every few hours.
SWAP_GB=2
if [ "$MEM_MB" -lt 1400 ]; then
  SWAP_GB=4
  warn "Under 1.5 GB of RAM. This is a B1s-sized VM. The gateway will run, but"
  warn "slowly, and first pairing can take several minutes. ${SWAP_GB} GB of swap will be added."
  warn "If it proves too tight, resize to B1ms or B2s - see deploy/azure/README.md."
fi
...
# ------------------------------------------------------------------ firewall
# Azure filters traffic in the network security group attached to the VM's
# subnet or NIC, not on the machine. There is nothing to open locally unless
# ufw happens to be enabled, which it is not on the stock Ubuntu image.
...
warn "Ports 80 and 443 must be allowed by the network security group in Azure."
warn "If https never answers, that is the first thing to check. deploy/azure/README.md has the command."
...
[ "$READY" = yes ] || warn "Not answering after ten minutes. On a small VM this can be slow; check: cd $DIR && docker compose logs openwa"
...
# Two consecutive failures before restarting, so a slow moment on a small VM
# is not treated as an outage. Checked over loopback, not the internet.
