...
/**
 * A promotional email in the agency's branding, with the unsubscribe notice
 * the law and the providers expect. Delivered inline because the caller is
 * already a background worker sending one batch at a time.
 */
export async function deliverCampaignEmail(input: CampaignEmailInput): Promise<boolean> {
  await ensureSchema()
  const sql = getSql()
  ...
  const paragraphs = input.body
    ...
    .map((t) => t.trim())
    .filter(Boolean)
  const rendered = renderEmail(brand, {
    ...
    cta: input.ctaUrl && input.ctaLabel ? { label: input.ctaLabel, url: input.ctaUrl } : undefined,
    footerExtra: {
      text: `You are receiving this because you are a client of ${brand.name}. To stop marketing emails,`,
      linkLabel: 'update your email preferences',
      linkUrl: `${SITE}/portal/profile`,
    },
  })
  const id = newId('eml')
  if (!emailConfigured()) {
    await sql`INSERT INTO email_log (id, organization_id, user_id, client_id, template, to_email, subject, status, error, related_type, related_id)
      VALUES (${id}, ${input.organizationId}, ${input.userId}, ${input.clientId}, 'campaign', ${input.to}, ${input.subject}, 'skipped', 'SMTP not configured', 'campaign', ${input.campaignId})`
    return false
  }
  await sql`INSERT INTO email_log (id, organization_id, user_id, client_id, template, to_email, subject, status, related_type, related_id)
    VALUES (${id}, ${input.organizationId}, ${input.userId}, ${input.clientId}, 'campaign', ${input.to}, ${input.subject}, 'queued', 'campaign', ${input.campaignId})`
  const result = await deliverEmail({ to: input.to, subject: input.subject, text: rendered.text, html: rendered.html, fromName: brand.name, replyTo: brand.supportEmail || undefined })
  if (result.ok) {
    await sql`UPDATE email_log SET status = 'sent', provider_id = ${result.messageId ?? null}, sent_at = now(), attempts = 1 WHERE id = ${id}`
    return true
  }
  await sql`UPDATE email_log SET status = 'failed', attempts = 1, error = ${result.error ?? 'unknown'} WHERE id = ${id}`
  return false
}

export interface DocumentEmailInput {
  key: 'quote-sent' | 'invoice-sent' | 'invoice-reminder'
  to: string
  organizationId: string
  userId?: string | null
  clientId?: string | null
...
/**
 * Sends a quote or invoice with its PDF attached. Delivered inline (the PDF
 * is already in memory and would have to be regenerated by a worker), logged
 * like every other email.
 */
export async function sendDocumentEmail(input: DocumentEmailInput): Promise<SendOutcome> {
  try {
    await ensureSchema()
    const sql = getSql()
    const rendered = await renderTemplate(input.key, input.organizationId, input.vars)
    if (!rendered) return 'failed'
    const id = newId('eml')
    const to = input.to.trim().toLowerCase()
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to)) return 'failed'
    if (!emailConfigured()) {
      await sql`INSERT INTO email_log (id, organization_id, user_id, client_id, template, to_email, subject, status, error, related_type, related_id)
        VALUES (${id}, ${input.organizationId}, ${input.userId ?? null}, ${input.clientId ?? null}, ${input.key}, ${to}, ${rendered.subject}, 'skipped', 'SMTP not configured', 'billing', ${input.relatedId ?? null})`
      return 'unconfigured'
    }
    await sql`INSERT INTO email_log (id, organization_id, user_id, client_id, template, to_email, subject, status, related_type, related_id)
      VALUES (${id}, ${input.organizationId}, ${input.userId ?? null}, ${input.clientId ?? null}, ${input.key}, ${to}, ${rendered.subject}, 'queued', 'billing', ${input.relatedId ?? null})`
    const result = await deliverEmail({
      to,
      subject: rendered.subject,
      text: rendered.text,
      html: rendered.html,
      fromName: rendered.brand.name,
      replyTo: rendered.brand.supportEmail || undefined,
    ...
    console.error('sendDocumentEmail failed', error instanceof Error ? error.message : error)
    return 'failed'
  }
}
