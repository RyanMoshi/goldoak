...
export function MobileNav({ open, onClose, organization, agent }: MobileNavProps) {
  ...
  return (
    <div className={cn('fixed inset-0 z-50 lg:hidden', open ? '' : 'pointer-events-none')} aria-hidden={!open}>
      <div className={cn('absolute inset-0 bg-forest/40 transition-opacity duration-200', open ? 'opacity-100' : 'opacity-0')} onClick={onClose} />
      <div
        ref={panelRef}
        role="dialog"
        aria-modal="true"
        ...
        className={cn(
          'absolute inset-y-0 left-0 flex w-[min(20rem,86vw)] flex-col bg-surface shadow-drawer transition-transform duration-200 ease-out',
