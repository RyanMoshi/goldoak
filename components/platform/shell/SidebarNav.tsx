...
import { agencyNavigation, agencySettings, type PlatformNavItem } from '@/data/platform/navigation'
...
function NavLink({ item, active, onNavigate }: { item: PlatformNavItem; active: boolean; onNavigate?: () => void }) {
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active ? 'bg-forest text-white' : 'text-ink-muted hover:bg-surface-2 hover:text-ink',
      )}
    >
      <Icon aria-hidden="true" className={cn('size-[18px] shrink-0', active ? 'text-gold' : 'text-ink-faint group-hover:text-ink')} strokeWidth={1.75} />
      <span className="truncate">{item.label}</span>
    </Link>
  )
}

export function SidebarNav({ onNavigate, withSettings = false, className }: { onNavigate?: () => void; withSettings?: boolean; className?: string }) {
