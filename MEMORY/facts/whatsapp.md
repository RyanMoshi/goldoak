...
- **One number per agency**: `whatsapp_channels` (organization_id, provider, session_id, phone, status, webhook_id). `lib/whatsapp/channels.ts`: `connectChannel` (creates an OpenWA session `org-<id>`, registers the webhook), `channelStatus` (syncs phone/status, returns the QR while pairing), `restartChannel`, `disconnectChannel`. UI `/agency/whatsapp` (+ `/api/whatsapp/channel` for live status), admin overview `/admin/channels`. The webhook resolves `sessionId` → organisation before routing; a channel's organisation overrides the contact's remembered agency. Outbound sends take `organizationId` → `providerForOrganization()`.
- **No more 2-hour expiry**: workflows expire after 30 days. After a gap of 6+ hours the bot replies "Welcome back… Here is where we were:" and repeats the current prompt (`currentPrompt()`), or shows the menu in consult mode.
- **Web chat** is server-owned (`services/webchat.ts`, key `web:<userId>`, `/api/chat` GET/POST, `web-answer` job fallback) so the thread survives refresh, reconnects and days away; the portal polls while an answer is pending.
- **Queue**: `enqueue()` kicks a background drain immediately (`kickQueue()`), so emails and WhatsApp jobs leave within seconds; the daily cron is the safety net (Hobby crons run once a day).
- Sign-up over WhatsApp issues a friendly temporary password (`generateTempPassword`) and emails it; first sign-in forces a change.
