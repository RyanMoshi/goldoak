...
import { SLATaskItem } from "@/components/dashboard/SLATaskItem";
import { Card, CardHeader } from "@/components/ui/Card";
import { EmptyState } from "@/components/ui/EmptyState";
import { insurerName } from "@/data/insurers";
import { cn } from "@/lib/cn";
import type { SLATask, TaskFilter, TaskType } from "@/types/dashboard";

interface SLATaskListProps {
  tasks: SLATask[];
  filter: TaskFilter;
...
function matches(task: SLATask, filter: TaskFilter): boolean {
  switch (filter) {
    case "all":
      return true;
    case "urgent":
      return task.sla === "overdue" || task.sla === "at-risk";
    case "today":
...
function noticeFor(task: SLATask): string {
  const insurer = task.insurer ? insurerName(task.insurer) : undefined;
  switch (task.action.kind) {
    case "follow-up":
      return `Follow-up drafted to ${insurer ?? "the insurer"} for ${task.client}. It is waiting for you in Quotes.`;
    case "review":
...
/**
 * The work queue. Filters are local state; completing a task removes it
 * after a short exit transition; primary actions confirm with a notice.
 */
export function SLATaskList({ tasks, filter, onFilterChange }: SLATaskListProps) {
  ...
  return (
    <Card as="section" flush aria-labelledby="needs-attention" className="animate-fade-up [animation-delay:120ms]">
      <div className="flex flex-col gap-3 px-4 pt-4 pb-3 sm:flex-row sm:items-start sm:justify-between sm:px-5">
        <CardHeader
          ...
          aside={
            <span
              data-numeric
              className="inline-flex h-6 min-w-6 items-center justify-center rounded-[4px] bg-forest px-1.5 font-mono text-[11.5px] font-semibold text-white"
              aria-label={`${visible.length} tasks shown`}
            >
              {visible.length}
            </span>
          }
        ...
        <div role="group" aria-label="Filter tasks" className="flex flex-wrap items-center gap-1.5">
          ...
          {isTypeFilter ? (
            <button
              ...
              className="inline-flex h-8 items-center gap-1.5 rounded-control border border-gold bg-gold-100 px-2.5 text-[12.5px] font-semibold text-gold-600 hover:bg-gold/20 focus-ring"
            >
      ...
      {notice ? (
        <div
          role="status"
          className="mx-4 mb-2 flex items-start gap-2 rounded-control border border-success/25 bg-success/8 px-3 py-2 text-[13px] text-ink sm:mx-5"
        >
          ...
          <button
            ...
            aria-label="Dismiss"
            className="rounded-control text-ink-muted hover:text-ink focus-ring"
          >
      ...
      {visible.length === 0 ? (
        <EmptyState
          ...
          description={
            open.length === 0
              ? "Every task with a service deadline is handled. New work appears here as it arrives."
        ...
        <ul className="divide-y divide-divider border-t border-line">
          {visible.map((task) => (
            <SLATaskItem
              ...
              onAction={(t) => setNotice(noticeFor(t))}
              onComplete={complete}
            />
          ))}
        </ul>
