...
/** The assistant's answer, rendered as a structured audit-style note rather than chat bubbles. */
export function AIResponsePanel({ status, command, result, errorMessage, onClose, onRetry }: AIResponsePanelProps) {
  return (
    <div role="status" aria-live="polite" className="p-3">
      <div className="flex items-start gap-2.5">
        <span className="mt-0.5 inline-flex size-6 shrink-0 items-center justify-center rounded-control bg-gold-100 text-gold-600">
        ...
        <button
          ...
          className="inline-flex size-7 shrink-0 items-center justify-center rounded-control text-ink-muted hover:bg-surface-2 hover:text-ink focus-ring"
        >
      ...
      <div className="mt-3 border-l-2 border-gold bg-surface-3 px-3.5 py-3">
        ...
        {status === "error" ? (
          <div>
            ...
            <button
              type="button"
              onClick={onRetry}
              className="mt-3 inline-flex items-center gap-1.5 rounded-control border border-line bg-surface px-2.5 py-1.5 text-[12.5px] font-semibold text-ink hover:border-ink-muted focus-ring"
            >
        ...
        {status === "done" && result ? (
          <div>
            <p className="font-serif text-[15px] leading-5 font-bold text-forest">{result.title}</p>
            <ul className="mt-2.5 divide-y divide-divider">
              {result.lines.map((line, i) => (
                <li key={i} className="flex items-baseline justify-between gap-4 py-1.5">
                  ...
                  {line.detail ? (
                    <span className="shrink-0 text-right font-mono text-[11.5px] text-ink-muted">{line.detail}</span>
                  ) : null}
                </li>
              ))}
            </ul>
