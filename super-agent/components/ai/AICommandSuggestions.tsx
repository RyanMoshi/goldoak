...
export function toOptions(recent: string[], suggestions: CommandSuggestion[], query: string): SuggestionOption[] {
  ...
  const recentOptions: SuggestionOption[] = q
    ? []
    : recent.map((text, i) => ({ id: `recent-${i}`, text, hint: "Recent", kind: "recent" }));
          ...
          <div key={option.id}>
            {showHeading ? (
              <div className="flex items-center justify-between px-3 pt-2 pb-1">
                <span className="label-caps text-ink-faint">
                  {option.kind === "recent" ? "Recent" : "Try"}
                </span>
                {option.kind === "recent" && hasRecent && onClearRecent ? (
                  <button
                    type="button"
                    onClick={onClearRecent}
                    className="rounded-control px-1 text-[11px] font-semibold text-ink-muted hover:text-ink focus-ring"
                  >
                    Clear
                  </button>
                ) : null}
            ...
            >
              {option.kind === "recent" ? (
                ...
                <Sparkles className={cn("size-4 shrink-0", highlighted ? "text-gold" : "text-gold")} aria-hidden="true" strokeWidth={1.75} />
              )}
              ...
              {highlighted ? (
                <CornerDownLeft className="size-3.5 shrink-0 text-white/70" aria-hidden="true" />
              ) : null}
            </div>
          </div>
        );
