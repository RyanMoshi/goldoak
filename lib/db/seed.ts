...
/**
 * Seeds the GoldOak organisation, one agency login and demo clients with
 * policies, quotes, claims, tasks and activity. Idempotent: every insert is
 * ON CONFLICT DO NOTHING, so running it twice changes nothing.
 */

const ORG = 'org_goldoak'
export const DEMO_PASSWORD_DEFAULT = 'GoldOak2026!'
...
export async function seedDatabase(password = DEMO_PASSWORD_DEFAULT): Promise<SeedSummary> {
  await ensureSchema()
  const sql = getSql()
  ...
  const users: [string, string, string, string, string, string, string | null][] = [
  ...
  const clients: [string, string | null, string, string, string, string | null, string][] = [
  ...
  const policies: [string, string, string, string, string, number | null, number, string, string, string, string][] = [
  ...
  const quotes: [string, string, string, string, string, number][] = [
  ...
  const submissions: [string, string, string, string, number | null, string][] = [
  ...
  const tasks: [string, string, string, string, string | null, string, string, string, string, number, boolean, number | null, string | null, string, string][] = [
  ...
  const counts = await sql`SELECT
    ...
    (SELECT count(*) FROM tasks WHERE completed_at IS NULL) AS tasks`
  const row = counts[0] ?? {}
  return { users: Number(row.users ?? 0), clients: Number(row.clients ?? 0), policies: Number(row.policies ?? 0), tasks: Number(row.tasks ?? 0) }
}
