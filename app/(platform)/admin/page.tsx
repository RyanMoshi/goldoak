...
export default async function AdminPage() {
  ...
  const tiles: { label: string; value: number | string; href?: string }[] = [
    ...
    { label: 'WhatsApp · AI', value: `${provider ? provider.name : 'off'} · ${aiConfigured() ? 'Claude' : 'catalogue'}` },
  ]
  ...
  return (
    <div className="animate-fade-up space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link href="/admin/conversations" className="inline-flex h-10 shrink-0 items-center gap-2 rounded-control border border-line bg-surface px-4 text-sm font-semibold text-ink hover:border-ink-muted focus-ring">
